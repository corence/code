...
type Upgrade = (String, Rational, String, Rational) -- name, price, target_property_name, multiplier
data PurchaseType = PTMilestone | PTUnit | PTUpgrade deriving Show
type PurchaseOrder = (PurchaseType, String, Rational, Rational) -- purchase_type, purchase_name, price, dps_increase
type PurchasePlan = (PurchaseOrder, Rational) -- purchase_order, time_to_breakeven
...
create_purchase_plan :: Rational -> Rational -> PurchaseOrder -> PurchasePlan
...
find_breakeven_time :: PurchaseOrder -> Rational -> Rational -> Rational
...
-- deprecated
find_upgrade_breakeven_time :: Upgrade -> [Property] -> Rational
...
find_upgrade_dps :: Upgrade -> [Property] -> Rational
...
xx1 = do
    putStrLn $ "1-----1"
    let milestone1 = ("Lemon Scented", "Newspaper", 150, "Lemonade Stand", 100 % 50)
    let lemonade = ("Lemonade Stand", 200, 1, 10 % 11, 2)
    let newspaper = ("Newspaper", 147, 10, 100 % 10, 8)
    let milestone1 = ("Lemon Scented", "Newspaper", 150, "Lemonade Stand", 100 % 50)
    putStrLn $ "find_milestone_price: " ++ (show $ fromRational $ find_milestone_price milestone1 [lemonade, newspaper])
    ...
    putStrLn $ "milestone_dps: " ++ (show $ fromRational $ find_milestone_dps milestone1 [newspaper, lemonade])
    putStrLn $ "dps increase: " ++ (show $ fromRational $ dps_increase)
    putStrLn $ "find_milestone_breakeven_time: " ++ (show $ fromRational $ find_milestone_breakeven_time milestone1 [newspaper, lemonade])
    ...
    let milestones = [("Combustible Lemons", "Lemonade Stand", 300, "Lemonade Stand", 2), ("Theoretical Economist", "All", 150, "All", 11), ("Lemon Scented", "Newspaper", 150, "Lemonade Stand", 100 % 50)]
